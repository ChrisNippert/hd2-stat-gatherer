...
- **Mission tally**: click to log each POI you find (Bunker, Container, Loot Pod) and what dropped from it (Medals, Requisition Slips, Super Credits, Weapons), plus Squad Mode, Difficulty, Faction, and a searchable Planet field (266 planets from the current galactic war map). This taxonomy is fixed, not user-editable — see below. For Medals, Requisition, and Super Credits, tap the exact amount picked up (e.g. Medals: +1/+2/+3) right there on the Tally page — this is the only place drop sizes are entered, and it feeds Drop Sizes and Resource Value on the Stats page automatically.
