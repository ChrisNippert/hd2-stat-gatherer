...
- **Mission tally**: tap **+1 FOUND** on a POI (Bunker, Container, Loot Pod) to log that you found one, which walks you straight into logging what was in each of its slots — pick the item, then (for Medals, Requisition, and Super Credits) the exact amount, repeating until the container's slots are full. Turn this off in Settings → **Guided Pickup Flow** (works the same on mobile or desktop) if you'd rather just mark it found and tally items yourself. Every item is also directly tappable/editable any time either way, independent of that guided flow. The interface adapts to your screen: on a narrow/mobile window, items are tap-to-open tiles; on a wide/desktop window, every item's controls are already visible inline. This is the only place drop sizes are entered, and it feeds Drop Sizes and Resource Value on the Stats page automatically. Also tag Squad Mode, Difficulty, Faction, and a searchable Planet field (266 planets from the current galactic war map) — this taxonomy is fixed, not user-editable, see below.
